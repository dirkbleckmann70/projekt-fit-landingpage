...
- **DB-Status ≠ Frontend-Status (wichtig für Finanz-/Anzeige-Code)** — `mapStatusForFrontend` liefert ans Frontend `strittig`→`disputed`, `reserviert`→`pending`, `abgeschlossen`→`completed`. Die Portal-Seiten (`admin/*.html`) vergleichen die GEMAPPTEN Werte (`disputed`/`completed`), NICHT die deutschen DB-Werte; `window.FIN_CANCELLED` enthält `disputed` (nicht `strittig`). Vor „Status X wird falsch behandelt": prüfen, welcher Wert nach dem Mapping ankommt. Beleg B-2026-06-15-14.
